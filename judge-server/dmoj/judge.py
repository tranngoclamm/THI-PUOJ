...
class Judge:
    ...
    def run_code_directly(
        ...
        # Tạo một "bài tập" giả chỉ để gọi grader
        ...
        try:
            grader = fake_problem.grader_class(
                None, fake_problem, language, utf8bytes(source)
            )
        ...
        try:
            # Tạo test case giả (cho phép truyền stdin)
            test_case = TestCase({
                'in': stdin,
                'out': '',  # không cần output mẫu
                'timelim': time_limit,
                'memlim': memory_limit,
                'points': 0
            })

            result: Result = grader.grade(test_case)
